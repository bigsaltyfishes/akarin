...
fn concurrent_tree() {
    ...
    let run = |tree: Arc<ConcurrentMap<u16, u16, 8>>, barrier: &std::sync::Barrier, low_bits| {
    ...
    let tree = Arc::new(ConcurrentMap::default());
...
fn big_scan() {
    ...
    let fill = |tree: Arc<ConcurrentMap<u32, u32>>,
    ...
    let read = |tree: Arc<ConcurrentMap<u32, u32>>, barrier: &std::sync::Barrier| {
        barrier.wait();
...
fn bulk_load() {
    ...
    let run = |tree: Arc<ConcurrentMap<u64, u64>>, barrier: &std::sync::Barrier, low_bits| {
        let shift = concurrency.next_power_of_two().trailing_zeros();
        let unique_key = |key| (key << shift) | low_bits;
