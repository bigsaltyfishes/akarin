...
/// Tests are placed here when the fuzzer finds bugs, so we can re-play them
/// deterministically even if tested on a machine without a fuzzer corpus loaded
/// etc...
...
fn map_model<const FANOUT: usize>(
    ...
    let map = ConcurrentMap::<_, _, FANOUT>::default();
...
#[test]
fn test_00() {
    let map = ConcurrentMap::<_, _, 8>::default();
    map.insert(0, 0);
    map.insert(103, 103);
