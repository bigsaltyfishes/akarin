...
mod alloc {
    ...
    unsafe impl std::alloc::GlobalAlloc for Alloc {
        unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
            ...
            unsafe { std::ptr::write_bytes(ret, 0xa1, layout.size()) };
            ret
        }

        unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
            unsafe { std::ptr::write_bytes(ptr, 0xde, layout.size()) };
...
fn leak_check() {
    ...
    let run = |tree: Arc<ConcurrentMap<u32, u32, 5>>, barrier: &std::sync::Barrier, low_bits| {
