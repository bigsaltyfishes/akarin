...
use libakarin_sync::{
    collections::skiplist::{SkipList, base},
    gc::{Collector, GarbageCollector},
};

use crate::collections::{CPU_NUM, GC};
...
#[test]
fn new() {
    SkipList::<i32, i32>::new(GC::global_handle());
    SkipList::<String, Box<i32>>::new(GC::global_handle());
}

#[test]
fn is_empty() {
    let guard = &GC::pin();
    let s = SkipList::new(GC::global_handle());
    assert!(s.is_empty());
...
fn get_or_insert_with_parallel_run() {
    ...
    let s = Arc::new(SkipList::new(GC::global_handle()));
    let s2 = s.clone();
...
#[test]
fn comparable_get() {
    use libakarin_sync::collections::skiplist::equivalent::{Comparable, Equivalent};
