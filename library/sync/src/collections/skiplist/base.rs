...
use crate::gc::{self, Atomic, Collector, Guard, Shared};
...
/// A skip list node.
///
/// This struct is marked with `repr(C)` so that the specific order of fields is
/// enforced. It is important that the tower is the last field since it is
/// dynamically sized. The key, reference count, and height are kept close to
/// the tower to improve cache locality during skip list traversal.
#[repr(C)]
struct Node<K, V> {
    /// The value.
    value: V,
...
impl<K, V> Node<K, V> {
    /// Allocates a node.
    ///
    /// The returned node will start with reference count of `ref_count` and the
    /// tower will be initialized with null pointers. However, the key and
    /// the value will be left uninitialized, and that is why this function
    /// is unsafe.
    ...
    /// Marks all pointers in the tower and returns `true` if the level 0 was
    /// not marked.
    fn mark_tower(&self) -> bool {
        ...
        for level in (0..height).rev() {
            let tag = unsafe {
                ...
                // `gc::unprotected()` in this situation.
                // TODO(Amanieu): can we use release ordering here?
                self.tower[level]
                    .fetch_or(1, Ordering::SeqCst, gc::unprotected())
                    .tag()
            };
    ...
    fn is_removed(&self) -> bool {
        let tag = unsafe {
            ...
            // `gc::unprotected()` in this situation.
            self.tower[0]
                .load(Ordering::Relaxed, gc::unprotected())
                .tag()
        };
        tag == 1
    }
    ...
    unsafe fn decrement(&self, guard: &Guard) {
            ...
            == 1
        {
            ...
            unsafe { guard.defer_retire(self as *const _ as *mut _, |ptr, _| Self::finalize(ptr)) }
        }
    }

    /// Decrements the reference count of a node, pinning the thread and
    /// destroying the node if the count become zero.
    #[inline]
...
// TODO(stjepang): Embed a custom `epoch::Collector` inside `SkipList<K, V>`.
// Instead of adding garbage to the default global collector, we should add it
// to a local collector tied to the particular skip list instance.
//
// Since global collector might destroy garbage arbitrarily late in the future,
// some skip list methods have `K: 'static` and `V: 'static` bounds. But a local
// collector embedded in the skip list would destroy all remaining garbage when
// the skip list is dropped, so in that case we'd be able to remove those bounds
// on types `K` and `V`.
//
// As a further future optimization, if `!mem::needs_drop::<K>() &&
// !mem::needs_drop::<V>()` (neither key nor the value have destructors),
// there's no point in creating a new local collector, so we should simply use
// the global one.
...
impl<K, V> SkipList<K, V> {
    ...
    ///
    /// If the skip list is being concurrently modified, consider the returned
    /// number just an approximation without any guarantees.
    ...
    K: Ord,
{
    ...
    /// Finds an entry with the specified key, or inserts a new `key`-`value`
    /// pair if none exist, where value is calculated with a function.
    ///
    ///
    /// <b>Note:</b> Another thread may write key value first, leading to the
    /// result of this closure discarded. If closure is modifying some other
    /// state (such as shared counters or shared objects), it may lead to
    /// <u>undesired behaviour</u> such as counters being changed without
    ...
    fn random_height(&self) -> usize {
        ...
        unsafe {
            ...
            // okay to use `gc::unprotected()` in this situation.
            while height >= 4
                && self.head[height - 2]
                    .load(Ordering::Relaxed, gc::unprotected())
                    .is_null()
    ...
    /// Searches for first/last node that is greater/less/equal to a key in the
    /// skip list.
    ///
    /// If `upper_bound == true`: the last node less than (or equal to) the key.
    ///
    /// If `upper_bound == false`: the first node greater than (or equal to) the
    /// key.
    ///
    ...
    /// Searches for a key in the skip list and returns a list of all adjacent
    /// nodes.
    ...
    ///
    /// If `replace` is `true`, then any existing entry with this key will first
    /// be removed.
        ...
        CompareF: Fn(&V) -> bool,
    {
        ...
        unsafe {
            ...
            // create value before creating node, so extra allocation doesn't happen if
            // value() function panics
            let value = value();
            // Create a new node.
            ...
            loop {
                ...
                if let Some(r) = search.found {
                    ...
                    if !replace {
                        ...
                        // If we couldn't increment the reference count, that
                        // means someone has
            ...
            // The new node was successfully installed. Let's create an entry associated
            // with it.
            let entry = RefEntry {
                parent: self,
                node: n,
...
where
    ...
    V: Send + 'static,
{
    /// Inserts a `key`-`value` pair into the skip list and returns the new
    /// entry.
    ///
    ...
    /// entry.
    ///
    /// If there is an existing entry with this key and compare(entry.value)
    /// returns true, it will be removed before inserting the new one.
        ...
        Q: ?Sized,
    {
        ...
        unsafe {
            ...
            loop {
                ...
                // First try incrementing the reference count because we have to return the node
                // as an entry. If this fails, repeat the search.
    ...
    pub fn clear(&self, guard: &mut Guard) {
        ...
        /// Number of steps after which we repin the current thread and unlink
        /// removed nodes.
...
impl<K, V> Clone for RefEntry<'_, K, V> {
    fn clone(&self) -> Self {
        unsafe {
            // Incrementing will always succeed since we're already holding a reference to
            // the node.
    ...
    fn drop(&mut self) {
        ...
        while !self.node.is_null() {
            unsafe {
                // Unprotected loads are okay because this function is the only one currently
                // using the skip list.
                let next = (&*self.node).tower[0].load(Ordering::Relaxed, gc::unprotected());
    ...
    fn next(&mut self) -> Option<(K, V)> {
        loop {
            ...
            unsafe {
                ...
                // The current node may be marked. If it is, it's been removed from the skip
                // list and we should just skip it.
                if next.tag() == 0 {
