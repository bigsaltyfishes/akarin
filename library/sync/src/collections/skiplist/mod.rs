...
//!
//! This mod provides the types [`SkipMap`] and [`SkipSet`]. Modified from
//! [crossbeam-skiplist](https://github.com/crossbeam-rs/crossbeam/tree/983d56b6007ca4c22b56a665a7785f40f55c2a53).
//! These data structures provide an interface similar to
//! [`BTreeMap`] and [`BTreeSet`], respectively, except they support safe
...
//! * The map would no longer be lock-free, inhibiting scalability and allowing
//!   for deadlocks.
//! * If a user of the map doesn't need mutable access, then they pay the price
//!   of locks without actually needing them.
//!
...
//!
//! To solve the above, this crate uses the _epoch-based memory reclamation_
//! mechanism implemented in [`crossbeam-epoch`]. Simplified, a value removed
//! from the map is not freed until after all references to it have been
//! dropped. This mechanism is similar to the garbage collection found in some
//! languages, such as Java, except it operates solely on the values inside the
//! map.
//!
//! This garbage collection scheme functions automatically; users don't have to
//! worry about it. However, keep in mind that holding [`Entry`] handles to
//! entries in the map will prevent that memory from being freed until at least
//! after the handles are dropped.
//!
...
//!
//! # Alternatives
//! This crate implements _ordered_ maps and sets, akin to [`BTreeMap`] and
//! [`BTreeSet`]. In many situations, however, a defined order on elements is
//! not required. For these purposes, unordered maps will suffice. In addition,
//! unordered maps often have better performance characteristics than their
...
//! * [`DashMap`](https://docs.rs/dashmap) implements a novel concurrent hash
//!   map with good performance characteristics.
//! * [`flurry`](https://docs.rs/flurry) is a Rust port of Java's
//!   `ConcurrentHashMap`.
//!
