...
pub struct SkipMap<K, V, G = GlobalGc>
where
    G: GarbageCollector,
{
    ...
    G: GarbageCollector,
{
    ...
    pub fn new() -> Self {
        Self {
            inner: base::SkipList::new(G::global_handle()),
    ...
    ///
    /// If the map is being concurrently modified, consider the returned number
    ...
    G: GarbageCollector,
{
    ...
    /// assert_eq!(*numbers.front().unwrap().value(), "five");
    /// ```
    pub fn front(&self) -> Option<Entry<'_, K, V, G>> {
        let guard = &G::pin();
        try_pin_loop(|| self.inner.front(guard)).map(Entry::<K, V, G>::new)
    }
    ...
    /// ```
    pub fn back(&self) -> Option<Entry<'_, K, V, G>> {
        let guard = &G::pin();
        try_pin_loop(|| self.inner.back(guard)).map(Entry::<K, V, G>::new)
    }
    ...
    /// ```
    pub fn get<Q>(&self, key: &Q) -> Option<Entry<'_, K, V, G>>
    where
        K: Comparable<Q>,
        Q: ?Sized,
    {
        let guard = &G::pin();
        try_pin_loop(|| self.inner.get(key, guard)).map(Entry::<K, V, G>::new)
    }
    ...
    /// ```
    pub fn lower_bound<'a, Q>(&'a self, bound: Bound<&Q>) -> Option<Entry<'a, K, V, G>>
    where
        K: Comparable<Q>,
        Q: ?Sized,
    {
        let guard = &G::pin();
        try_pin_loop(|| self.inner.lower_bound(bound, guard)).map(Entry::<K, V, G>::new)
    }
    ...
    /// ```
    pub fn upper_bound<'a, Q>(&'a self, bound: Bound<&Q>) -> Option<Entry<'a, K, V, G>>
    where
        K: Comparable<Q>,
        Q: ?Sized,
    {
        let guard = &G::pin();
        try_pin_loop(|| self.inner.upper_bound(bound, guard)).map(Entry::<K, V, G>::new)
    }
    ...
    /// ```
    pub fn get_or_insert(&self, key: K, value: V) -> Entry<'_, K, V, G> {
        let guard = &G::pin();
        Entry::<K, V, G>::new(self.inner.get_or_insert(key, value, guard))
    }
    ...
    /// assert_eq!(*jobs_age.value(), 65);
    /// ```
    pub fn get_or_insert_with<F>(&self, key: K, value_fn: F) -> Entry<'_, K, V, G>
    where
        F: FnOnce() -> V,
    {
        let guard = &G::pin();
        Entry::<K, V, G>::new(self.inner.get_or_insert_with(key, value_fn, guard))
    }
    ...
    /// }
    /// ```
    pub fn range<Q, R>(&self, range: R) -> Range<'_, Q, R, K, V, G>
    where
        R: RangeBounds<Q>,
        K: Comparable<Q>,
        Q: ?Sized,
    ...
    G: GarbageCollector,
{
    ...
    /// ```
    pub fn insert(&self, key: K, value: V) -> Entry<'_, K, V, G> {
        let guard = &G::pin();
        Entry::<K, V, G>::new(self.inner.insert(key, value, guard))
    }
    ...
    /// ```
    pub fn compare_insert<F>(&self, key: K, value: V, compare_fn: F) -> Entry<'_, K, V, G>
    where
        F: Fn(&V) -> bool,
    {
        let guard = &G::pin();
        Entry::<K, V, G>::new(self.inner.compare_insert(key, value, compare_fn, guard))
    }
    ...
    /// ```
    pub fn remove<Q>(&self, key: &Q) -> Option<Entry<'_, K, V, G>>
    where
        K: Comparable<Q>,
        Q: ?Sized,
    {
        let guard = &G::pin();
        self.inner.remove(key, guard).map(Entry::<K, V, G>::new)
    }
    ...
    /// ```
    pub fn pop_front(&self) -> Option<Entry<'_, K, V, G>> {
        let guard = &G::pin();
        self.inner.pop_front(guard).map(Entry::<K, V, G>::new)
    }
    ...
    /// assert!(numbers.is_empty());
    /// ```
    pub fn pop_back(&self) -> Option<Entry<'_, K, V, G>> {
        let guard = &G::pin();
        self.inner.pop_back(guard).map(Entry::<K, V, G>::new)
    }
...
impl<K, V, G> Default for SkipMap<K, V, G>
where
    G: GarbageCollector,
{
    fn default() -> Self {
...
impl<K, V, G> fmt::Debug for SkipMap<K, V, G>
where
    K: Ord + fmt::Debug,
    V: fmt::Debug,
    G: GarbageCollector,
...
impl<K, V, G> IntoIterator for SkipMap<K, V, G>
where
    G: GarbageCollector,
{
    type Item = (K, V);
...
impl<'a, K, V, G> IntoIterator for &'a SkipMap<K, V, G>
where
    K: Ord,
    G: GarbageCollector,
{
    type Item = Entry<'a, K, V, G>;
    type IntoIter = Iter<'a, K, V, G>;

    fn into_iter(self) -> Iter<'a, K, V, G> {
        self.iter()
    }
}

impl<K, V, G> FromIterator<(K, V)> for SkipMap<K, V, G>
where
    K: Ord,
    G: GarbageCollector,
{
...
pub struct Entry<'a, K, V, G = GlobalGc>
where
    G: GarbageCollector,
{
    inner: ManuallyDrop<base::RefEntry<'a, K, V>>,
    __marker: core::marker::PhantomData<G>,
}

impl<'a, K, V, G> Entry<'a, K, V, G>
where
    G: GarbageCollector,
{
...
impl<K, V, G> Drop for Entry<'_, K, V, G>
where
    G: GarbageCollector,
{
    fn drop(&mut self) {
        unsafe {
            ManuallyDrop::into_inner(ptr::read(&self.inner)).release_with_pin(G::pin);
        }
    }
}
    ...
    G: GarbageCollector,
{
    ...
    pub fn next(&self) -> Option<Entry<'a, K, V, G>> {
        let guard = &G::pin();
        self.inner.next(guard).map(Entry::new)
    }

    /// Returns the previous entry in the map.
    pub fn prev(&self) -> Option<Entry<'a, K, V, G>> {
        let guard = &G::pin();
        self.inner.prev(guard).map(Entry::new)
    }
}

impl<K, V, G> Entry<'_, K, V, G>
where
    K: Ord + Send + 'static,
    V: Send + 'static,
    G: GarbageCollector,
...
impl<K, V, G> Clone for Entry<'_, K, V, G>
where
    G: GarbageCollector,
{
    fn clone(&self) -> Self {
...
impl<K, V, G> fmt::Debug for Entry<'_, K, V, G>
where
    K: fmt::Debug,
    V: fmt::Debug,
    G: GarbageCollector,
...
pub struct Iter<'a, K, V, G = GlobalGc>
where
    G: GarbageCollector,
{
    ...
    G: GarbageCollector,
{
    ...
    fn next(&mut self) -> Option<Entry<'a, K, V, G>> {
        let guard = &G::pin();
        self.inner.next(guard).map(Entry::new)
    }
}

impl<'a, K, V, G> DoubleEndedIterator for Iter<'a, K, V, G>
where
    K: Ord,
    G: GarbageCollector,
{
    fn next_back(&mut self) -> Option<Entry<'a, K, V, G>> {
        let guard = &G::pin();
...
impl<K, V, G> Drop for Iter<'_, K, V, G>
where
    G: GarbageCollector,
{
    fn drop(&mut self) {
...
pub struct Range<'a, Q, R, K, V, G = GlobalGc>
where
    K: Ord + Comparable<Q>,
    R: RangeBounds<Q>,
    Q: ?Sized,
...
impl<'a, Q, R, K, V, G> Iterator for Range<'a, Q, R, K, V, G>
where
    K: Ord + Comparable<Q>,
    R: RangeBounds<Q>,
    Q: ?Sized,
...
impl<'a, Q, R, K, V, G> DoubleEndedIterator for Range<'a, Q, R, K, V, G>
where
    K: Ord + Comparable<Q>,
    R: RangeBounds<Q>,
    Q: ?Sized,
...
impl<Q, R, K, V, G> fmt::Debug for Range<'_, Q, R, K, V, G>
where
...
impl<Q, R, K, V, G> Drop for Range<'_, Q, R, K, V, G>
where
    K: Ord + Comparable<Q>,
    R: RangeBounds<Q>,
    Q: ?Sized,
