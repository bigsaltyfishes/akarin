...
use crate::gc::{Atomic, Guard, Shared, guard::unprotected};
...
pub enum IterError {
    ...
    /// that this iterator was inspecting. Subsequent iteration will restart
...
impl<T, C: IsElement<T>> List<T, C> {
    ...
    pub unsafe fn insert<'g>(&'g self, container: Shared<'g, T>, guard: &'g Guard) {
        ...
        loop {
            // Set the Entry of the to-be-inserted element to point to the previous
            // successor of `to`.
    ...
    ///    returned.
    /// 2. If an object is deleted during iteration, it may or may not be
    ///    returned.
    /// 3. The iteration may be aborted when it lost in a race condition. In
    ///    this case, the winning thread will continue to iterate over the same
    ///    list.
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while let Some(c) = unsafe { self.curr.as_ref() } {
            ...
            if succ.tag() == 1 {
                ...
                // The tag should always be zero, because removing a node after a logically
