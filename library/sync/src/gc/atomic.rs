...
impl<T: ?Sized + Pointable> Atomic<T> {
    ...
    /// Loads a `Shared` from the atomic pointer using a "consume" memory
    /// ordering.
    ///
    ...
    /// for the read-modify-write operation that takes place if the
    ...
    /// fails. Using `Acquire` as success ordering makes the store part
    /// of this operation `Relaxed`, and using `Release` makes the successful
    /// load `Relaxed`. The failure ordering can only be `SeqCst`, `Acquire`
    /// or `Relaxed` and must be equivalent to or weaker than the success
    /// ordering.
    ///
    /// # Examples
    ///
    ...
    /// with different tags, will not be considered equal.
    ///
    /// Unlike [`compare_exchange`], this method is allowed to spuriously fail
    ...
    /// Fetches the pointer, and then applies a function to it that returns a
    /// new value. Returns a `Result` of `Ok(previous_value)` if the
    /// function returned `Some`, else `Err(_)`.
    ///
    /// Note that the given function may be called multiple times if the value
    /// has been changed by other threads in the meantime, as long as the
    ...
    /// failure orderings of [`Atomic::compare_exchange`] respectively.
    ///
    ...
    pub fn fetch_and<'g>(&self, val: usize, order: Ordering, _: &'g Guard) -> Shared<'g, T> {
        ...
        // use AtomicPtr::fetch_* in all cases from the version in which it is
        // stabilized.
        unsafe {
    ...
    ///
    /// This consumes the atomic and converts it into [`Owned`]. As [`Atomic`]
    /// doesn't have a destructor and doesn't drop the pointee while
    /// [`Owned`] does, this is suitable for destructors of data structures.
    ///
    /// # Panics
    ///
    ...
    ///
    /// This method may be called only if the pointer is valid and nobody else
    /// is holding a reference to the same object, or the pointer is null.
    ///
    /// # Examples
    ///
    /// ```ignore
...
/// `crossbeam-epoch`.
pub trait Pointer<T: ?Sized + Pointable>: Sealed {
    ...
    /// # Safety
    ///
    /// The given `data` should have been created by `Pointer::into_ptr()`, and
    /// one `data` should not be converted back by `Pointer::from_ptr()`
    /// multiple times.
...
impl<'g, T: ?Sized + Pointable> Shared<'g, T> {
    ...
    /// Dereferences the pointer.
    ///
    /// Returns a mutable reference to the pointee that is valid during the
    /// lifetime `'g`.
    ///
    /// # Safety
    ///
    /// * There is no guarantee that there are no more threads attempting to
    ///   read/write from/to the actual object at the same time.
    ///
    ///   The user must know that there are no concurrent accesses towards the
    ///   object itself.
    ///
    ...
    ///
    /// This method may be called only if the pointer is valid and nobody else
    /// is holding a reference to the same object, or if the pointer is
    /// null.
    ///
    /// # Examples
    ///
